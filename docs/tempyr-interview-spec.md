...
File: local `.claude/settings.json`, generated by `tempyr init` / `tempyr update`
(relevant excerpt; see `docs/claude-settings.example.json` for the full
managed template)
...
```json
{
  "hooks": {
    "PostToolUse": [
      {
        "matcher": "mcp__tempyr__graph_add_node|mcp__tempyr__graph_update_node|mcp__tempyr__graph_add_edge|mcp__tempyr__interview_commit",
        "hooks": [
          {
            "type": "command",
          ...
          {
            "type": "command",
            "command": "tempyr index update --json"
          }
        ]
      }
    ]
...
This ensures graph validation and index refresh run after every mutation:
node creation, node update, edge addition, or interview commit. Deterministic,
100% execution, no LLM involvement.
