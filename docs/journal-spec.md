...
| **4. Polish** | ✅ Shipped (PRs #29, #30, #31, #32) | Auto-emit on task status transitions (4a) and interview lifecycle (4b), managed local `.claude/settings.json` SessionStart/SessionEnd hooks + `tempyr journal bootstrap`/`finalize` (4c), MCP annotations across all tools (4c), `tempyr doctor` journal section (4c), CLAUDE.md/AGENTS.md journal section (4d) |
...
- ~~**Stats dashboard**~~ — *implemented post-Phase 4.* `tempyr journal stats` and the `journal_stats` MCP tool return aggregate counts: total entries / sessions / agents, kind distribution, provisional-vs-final ratio, dead-end ratio (with a callout when below 10% — agents likely aren't logging failures), sessions per agent, top tags, top files, and a per-day activity histogram for the last N days. Optional `--since-days` scopes the totals; CLI also renders an ASCII activity bar chart.
