...
use graphforge_index::embeddings::{self, EmbeddingConfig};
...
pub fn run_stats(ctx: &ProjectContext, json: bool) -> anyhow::Result<()> {
    ...
    let emb_count = index.embedding_count().unwrap_or(0);
...
fn try_embed(
    ...
    let stats = rt.block_on(embeddings::embed_graph(index, graph, provider.as_ref()))?;
    Ok(stats)
}

/// Load embedding config from .graphforge/config.toml, falling back to defaults.
fn load_embedding_config(ctx: &ProjectContext) -> EmbeddingConfig {
    let config_path = ctx.graphforge_dir.join("config.toml");
    if let Ok(content) = std::fs::read_to_string(&config_path) {
        // Try to extract the [embedding] section
