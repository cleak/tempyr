...
pub fn run(
    ...
    let local_path = ctx.graphforge_dir.join("render").join(format!("{template_name}.toml"));
    let result = if local_path.exists() {
        graphforge_render::render(&graph, &local_path, root_id, &temporal_filter)?
    } else {
        ...
        let template_toml = match template_name {
            ...
            "tdd" => BUILTIN_TDD,
            _ => anyhow::bail!(
                "Unknown template: '{template_name}'. Available: prd, tdd (or place a custom template in .graphforge/render/)"
            ),
        };
        graphforge_render::render_from_str(&graph, template_toml, root_id, &temporal_filter)?
    };
